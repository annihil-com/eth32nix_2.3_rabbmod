...
void CTools::Init(int Game)
{
	...
	env = getenv("ETH32_SCRIPTS");
	if (env) Perl.GetPerlScripts(env);
	...
	env = getenv("ETH32_NO_AUTH");
	eth32.authEnabled = true;
	if (env)
		eth32.authEnabled = strcasecmp(env,"true") ? true : false;
	...
	Tools.LoadUserCvars();
	eth32.authed = eth32.authEnabled ? Portal.Auth() : false;

#ifdef ETH32_DEBUG
	if (eth32.authEnabled)
		Debug.Log("Auth status: %s", eth32.authed ? "Information validated" : "Failed");
	else
		Debug.Log("Auth, IRC, tracker disabled");
#endif

	if (!eth32.authed && eth32.authEnabled)
		FATAL_ERROR("\nEth32 Auth failed, check your auth.ini and that your username/password are correct\n"
						"If you don't have a username or password, register now at\n\n"
						"\t\teth32.chaplja.com\n\n"
						"If you really don't want to Auth, export ETH32_NO_AUTH before starting the game\n"
						"Note: you won't be able to use the tracker or IRC!\n")
