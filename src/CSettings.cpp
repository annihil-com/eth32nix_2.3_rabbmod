...
void CSettings::Save(const char *filename)
{
	...
	if(!eth32.settings.defaultSoundsLoaded)
	{
		...
		for(int t = 0; t < SOUND_LEVEL_MAX; t++)
		{
			...
			{
				sprintf(buf1, "sound%i%i", t, s);
				sprintf(buf2, "%s", defaultSounds[soundName]);

				Tools.WritePrivateProfileString("Sound Settings", buf1, buf2, (char *)filename);
				soundName++;
			}

		}
		Tools.WritePrivateProfileString("Main Settings", "loaddefaultsounds", "true", (char*)filename);
	}
}
