...
//void eth32_get_offsets(void *p, void *b) 
		...
		if (supported == -1){
			if (strstr(filename, "etace") || strstr(filename, "etpub") || strstr(filename, "etmain") || strstr(filename, "shrub") ) {
				...
				offset_t eth32_get_offsets = (offset_t)dlsym(result, "eth32_get_offsets");
...
void *eth32_dlsym(void *handle, const char *symbol) {
	...
	if (eth32.pb.loaded && !strcmp(symbol, "ca")) 
		Hook.InitPB();
...
void CHook::InitCGameHook()
{
	...
	if (eth32.eth32mod) {
		...
		ver_t eth32_clientdll_version = (ver_t)dlsym(eth32.cg.handle, "eth32_clientdll_version");
		...
		typedef void (*offset_t)(void *p, void *b);
		offset_t eth32_get_offsets = (offset_t)dlsym(eth32.cg.handle, "eth32_get_offsets");
...
{
	//Detour.GOTHook("memset", (void*)eth32_memset);
	Detour.GOTHook("dlsym", (void *)eth32_dlsym);
	Detour.GOTHook("dlopen", (void *)eth32_dlopen);
	Detour.GOTHook("dlclose", (void *)eth32_dlclose);
...
void CHook::hookGame(bool state)
{
	...
	if (state && !hooked)
	{
		...
		if (eth32.et->etroq_video){
			...
			unprotect((void*)0x080a6616);
	memset((void*)0x080a6616, 0x90, 5);
	reprotect((void*)0x080a6616);
...
#ifdef ETH32_DEBUG
	Debug.Log("FS_Startup2: adding %s", eth32.path);
#endif
}
...
int CDECL eth32_CG_vmMain(int command, ...)
{
	...
	switch (command)
	{
		case CG_CONSOLE_COMMAND:
		{
			...
			perlScript_t *pl = perl_scripts;
			while (pl) {
				if (!strcmp((char*)u, pl->basename))
					break;
				pl = pl->next;
			}

			if (!pl) {
				Syscall.CG_Print("^3Warning: attempted to run non-existent script %s\n", u);
				return 1;
			}

			Perl.ParsePerlCommand(pl);
...
int CDECL eth32_UI_vmMain(int command, ...)
{
	...
	switch (command)
	{
		...
		case UI_CONSOLE_COMMAND:
		{
			...
			if (!pl) {
				Syscall.UI_Print("^3Warning: attempted to run non-existent script %s\n", u);
				return 1;
			}
				...
				if (!strcmp(var_name, cv->name)) {					
					...
					if (cv->type == PB_CVAR_IN) {
						if (!cvarValid(var_name, cv)) {
							snprintf(buffer, sizeof(buffer), "%f", cv->high);
							return buffer;					
						}
					} else if (cv->type == PB_CVAR_OUT) {
						if (!cvarValid(var_name, cv)) {
