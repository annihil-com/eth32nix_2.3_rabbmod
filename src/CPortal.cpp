...
extern void *PortalThread(void *arg);
uint32_t nextrnd;
...
int _b64w_encode(char* dest, const char* str, int len)
{
    int i;
...
int _b64w_decode(char* dest, const char* src, int len)
{
	if (len == 0) return 0;
    /*
...
int pencode(uchar *out, uchar *in, int len, int key)
{
...
uchar *pdecode(uchar *in, int len)
{
	// assume key length 24bit
...
bool CPortal::Auth()
{
	...
	sprintf(authFile, "%s/%s", eth32.path, AUTH_FILE);

	Tools.GetPrivateProfileString("Auth", "Username", "demoUser", this->username, sizeof(this->username), (char *)authFile);
	Tools.GetPrivateProfileString("Auth", "Password", "demoPass", this->password, sizeof(this->password), (char *)authFile);

	// replace username with us, password with pw, etc...
	char msg2[512];
	char msg[512];
	sprintf(msg,"us=%s&pw=%s&cl=%s&crc=0x%x", this->username, this->password, "2.0", eth32.eth32CRC);
...
void CPortal::Init()
{
	...
	if (!init) {
		...
		if (eth32.authEnabled) {
			if (pthread_create(&this->trackerThread, NULL, PortalThread, NULL))
				fprintf(stderr, "Portal: error creating tracker thread\n");
		}

		init = true;
#ifdef ETH32_DEBUG
		if (eth32.authEnabled) Debug.Log("Portal Thread initialised!");
#endif
	}
}
...
void CPortal::UpdateServers(int usernum)
{
	...
	if (numServers < MAX_PORTAL_SERVERS) {
		strcpy(servers[numServers].serverAddr, u->server);
		strcpy(servers[numServers].serverName, u->serverName);
		strcpy(servers[numServers].serverMapName, u->serverMapName);
...
bool CPortal::ParseUserInfo(const char *string, int num)
{
	...
	while (userstring.hasMoreTokens()) {
		...
		if (datatype == 'u') {
			...
			if (!strcmp(data.c_str(), this->username))
				return false;
			strncpy(users[num].username, data.c_str(), 64);
		}
		else if (datatype == 'i')
			strncpy(users[num].server, data.c_str(), 32);
		else if (datatype == 'e')
			...
			strncpy(users[num].serverName, data.c_str(), 50);
		else if (datatype == 'z')
			strncpy(users[num].serverMapName, data.c_str(), 25);
	}
	...
	if (eth32.cgameLoaded && !strcmp(eth32.server, users[num].server) && users[num].clientNum >= 0 && users[num].clientNum < MAX_CLIENTS) {
		...
		strcpy(users[num].player->portalName, users[num].username);
...
void CPortal::RetrieveUsers(void)
{
	...
	int num;
	sscanf(userinfo.nextToken().c_str(), "%i", &num);
	int validNum = 0;
	...
	if (eth32.cgameLoaded && eth32.settings.portalRampageNotify) {
		for (int i=0 ; i<numServers ; i++)
		{
			if (servers[i].rampage && strcmp(eth32.server, servers[i].serverAddr)) {
				...
				sprintf(msg, "^1RAMPAGE in progress!!!\n^o%s\n^2( ^o%i ^dusers ^2| ^o%i ^dfriends ^2)",
...
void CPortal::DebugExtraUsers(int num)
{
	...
	{
		sprintf(users[i].username, "tempuser%02i", i);
		sprintf(users[i].server, "%i.%i.%i.%i:%i", rand()%256, rand()%256, rand()%256, rand()%256, rand()%64000);
		sprintf(users[i].serverMapName, "Baserase v%i", i);
		sprintf(users[i].serverName, "Some Server %02i", i);
...
{
	Tools.GetPrivateProfileString("Main Settings", "friends", "\0", friends, sizeof(friends), (char *)filename);
}

void CPortal::SaveSettings(const char *filename)
{
	Tools.WritePrivateProfileString("Main Settings", "friends", friends, (char *)filename);
}
...
void CPortal::ConsoleList(void)
{
	...
	if (numServers) {
		...
		for (int i=0 ; i<numServers ; i++) {
			...
			strncpy(sName, s->serverName, sizeof(sName)-1);
			strncpy(sMap, s->serverMapName, sizeof(sMap)-1);
			strncpy(sAddr, s->serverAddr, sizeof(sAddr)-1);
	...
	if (numUsers) {
		...
		for (int i=0 ; i<numUsers ; i++) {
			if (users[i].anon) {
				...
				strncpy(serverName, users[i].serverName, 25);
...
void CPortal::ConsoleJoin(void)
{
	if (Syscall.UI_Argc() == 2) {
		...
		// user specified a name
			for (int i=0 ; i<numUsers ; i++)
			{
				if (!strncasecmp(users[i].username, arg, strlen(arg))) {
					if (users[i].anon) {
						Syscall.UI_Print("\n^1ERROR: ^oUser ^d%s ^ois anonymous, unable to join!\n\n", users[i].username);
						return;
					}
