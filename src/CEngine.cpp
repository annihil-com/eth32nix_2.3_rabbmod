...
void CEngine::RegisterMedia()
{
	...
	eth32.cg.media.chams[CHAMS_KANKER] = Syscall.R_RegisterShader("kankerChams");
	eth32.cg.media.chams[CHAMS_INVI] = Syscall.R_RegisterShader("inviChams");
	eth32.cg.media.chams[CHAMS_SPARKLE] = Syscall.R_RegisterShader("sparkleChams");
...
void CEngine::AutoPush()
{
	...
	if (eth32.settings.autoPush && Engine.forward){
		if (eth32.settings.aimprotect && !eth32.cg.nspectators ){
			trace_t tr;
			vec3_t end;
...
//		Syscall.R_AddLightToScene(end, 300.0f, 5.0, 1.0, 0.0, 0.0, 0, REF_FORCE_DLIGHT);
	...
	else if (eth32.settings.hasOutlineShader){
